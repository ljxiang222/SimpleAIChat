...
export default function Message({ text, sender, scrollBottom }: MessageProps) {
  ...
  return (
    <div
      className={`mb-2 transition-opacity duration-300 ${sender === "user" ? "text-left flex flex-row justify-end" : "text-left bg-[#ffffff]"}`}
    >
      {
        sender == "user" ? (<div className=" bg-[#eff6ff] px-4 py-2 rounded-lg w-auto markdown-container ">
          {visibleText}
        </div>) : (
          <ReactMarkdown
